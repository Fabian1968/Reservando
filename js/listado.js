...
/*
GUÍA 2 - PASO 6.
Se modifica la fn buscarRestaurante(id) aplicando la fn find().
*/
//Dado un id, busca el objeto del listado que tiene ese id
Listado.prototype.buscarRestaurante = function(id) {
    const resultado = this.restaurantes.find(resto => resto.id === id);
    if (resultado) {
        return resultado;
    } else {
...
/*
GUÍA 2 - PASO 4.
Método que elimina los elementos que se repiten en un array.
*/
Listado.prototype.eliminarRepetidosEnUnArreglo = function(array) {
    let sinRepetidos = array.filter((valor, indiceActual, arreglo) => arreglo.indexOf(valor) === indiceActual);
    return sinRepetidos;
}

/*
GUÍA 2 - PASO 3 Y PASO 5.
Se modifican nombres de funciones y variables por nombres más declarativos.
Se refactorizan las siguientes fns utilizando la fn map().
*/
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    ubicaciones = this.restaurantes.map(ciudad => ciudad.ubicacion);
    ubicacionesSinRepetirse = Listado.prototype.eliminarRepetidosEnUnArreglo(ubicaciones);
    return ubicacionesSinRepetirse.sort();
}
...
Listado.prototype.obtenerRubros = function() {
    let rubros = [];
    rubros = this.restaurantes.map(especialidad => especialidad.rubro);
    var rubrosSinRepetirse = Listado.prototype.eliminarRepetidosEnUnArreglo(rubros);
...
Listado.prototype.obtenerHorarios = function() {
    let arregloDeHorarios = [];
    arregloDeHorarios = this.restaurantes.map(hora => hora.horarios);
    //En este arreglo vamos a poner todos los horarios, uno por uno.
    var TodosLosHorarios = [];
    arregloDeHorarios.forEach(function(arreglo) {
        arreglo.forEach(function(horario) {
            TodosLosHorarios.push(horario)
        });
    });
    horariosSinRepetidos = Listado.prototype.eliminarRepetidosEnUnArreglo(TodosLosHorarios);
    return horariosSinRepetidos.sort();
}
